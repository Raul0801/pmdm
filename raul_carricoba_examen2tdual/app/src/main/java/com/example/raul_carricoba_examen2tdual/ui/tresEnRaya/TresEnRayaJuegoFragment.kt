...
import com.example.raul_carricoba_examen2tdual.databinding.FragmentJuegoBinding

class TresEnRayaJuegoFragment : Fragment() {
    ...
    private fun onCellClicked(button: Button) {
        ...
        if (modelo.jugarTurno(row, col)) {
            ...
            modelo.ganador?.let{
                findNavController().navigate(TresEnRayaJuegoFragmentDirections.actionNavJuegoToNavGanador(it.toString()))
                reset()
            }
        }
    }
